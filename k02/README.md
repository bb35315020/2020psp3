...
## ソースコードの説明
Node* InsertNewNode(City newCity, Node* CurrentTop)//必要なメモリを確保しリストの先頭に新しいデータを挿入する
{
    Node* nude = malloc(sizeof(Node));
    
    nude->pNext = CurrentTop;// 構造体の実体のポインタ変数名->メンバ名。
    //ポインタ(nude)に(pNect)を使ってアクセスしアドレス先の値を初期化
    //->はアロー演算子
    nude->city = newCity;
    //nudeにcityを使ってアクセスしてアドレス先の値を初期化
...
int SearchCityByID(Node* pList, int ID, City* pCity)//ＩＤをキーとしリストを検索
{
    int i = 0;
    /*pListとNULLが一致するまで探索*/
    while(pList != NULL){
        if(pList->city.id==ID){
            /*探索したいＩＤとpListが一致したらiを返す*/
            *pCity=pList->city;
...
C:\Users\bb35315020\Downloads\プログラミング演習Ⅲ\2020psp3\k02>.\k02
